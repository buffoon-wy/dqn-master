...
def optimize_model():
    ...
    a = batch.next_state
    maps = map(lambda s: s is not None, a)
    non_final_mask = torch.tensor(tuple(maps),
                                  device=device, dtype=torch.bool)
    non_final_next_states = torch.cat([s for s in a if s is not None])
    state_batch = torch.cat(batch.state)
    action_batch = torch.cat(batch.action)
    reward_batch = torch.cat(batch.reward)

    actions = policy_net(state_batch)
    state_action_values = actions.gather(1, action_batch)      # (32, 1)
    next_state_values = torch.zeros(BATCH_SIZE, device=device)      # 32
...
def random_start(skip_steps=30, m=1):
    env.reset()
